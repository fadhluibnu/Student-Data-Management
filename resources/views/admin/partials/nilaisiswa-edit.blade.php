...
<input type="hidden" name="jenis_ujian" value="{{ request('jenis_ujian') }}">
<input type="hidden" name="tahun_ujian" value="{{ request('tahun_ujian') }}">
<div class="mb-3">
    <label for="nis" class="form-label">NIS</label>
    <input type="text" name="nis" class="form-control @error('nis') is-invalid @enderror" id="nis" placeholder="NIS"
        required disabled value="{{ $siswa->user->nis }}">
    @error('nis')
...
<div class="mb-3">
    ...
    <input type="text" name="name" class="form-control  @error('name') is-invalid @enderror" id="nama" placeholder="name"
        required disabled value="{{ $siswa->user->name }}">
    @error('name')
        <div class="  invalid-feedback">
            {{ $message }}
        </div>
...
<div class="mb-3">
    ...
    <input type="text" name="jawa" class="form-control  @error('jawa') is-invalid @enderror" id="jawa"
        placeholder="Bahasa Jawa" value="{{ $siswa->jawa }}">
    @error('jawa')
        <div class="  invalid-feedback">
            {{ $message }}
        </div>
...
<div class="mb-3">
    ...
    <input type="text" name="pjok" class="form-control  @error('pjok') is-invalid @enderror" id="pjok"
        placeholder="PJOK" value="{{ $siswa->pjok }}">
    @error('pjok')
        <div class="  invalid-feedback">
            {{ $message }}
        </div>
